...
class Disassembler:
  ...
  def decode(self):
    mem = '000000' + str(self.PRG[self.pc])
    try:
      instr = decoder[mem[-2:]]
    except KeyError:
      instr = decoder[None]
    self.pc += 1
    return instr, mem[-3:-(3 + instr.num_param):-1], mem[-(3 + instr.num_param)] if instr.writes else None

  def step(self, start = None):
    if start is not None: self.pc = start
    m2s = {'0': '*', '1': '', '2': '!'}
    w2s = {'0': '*', '2':'!'}
    instr, read_modes, write_mode = self.decode()
    params = ' '.join('{}{}'.format(m2s[m], p) for m, p in zip(read_modes, self.PRG[self.pc:self.pc + instr.num_param]))
    addr = '-> {}{}'.format(m2s[write_mode], self.PRG[self.pc + instr.num_param]) if instr.writes else ''
    print('{:04} {} {} {}'.format(self.pc, instr.name, params, addr))
    self.pc += instr.num_param + instr.writes
...
class Computer:
  ...
  def step(self):
    if DEBUG:
      self.d.step(self.pc)
      premem = dict(enumerate(self.MEM))
    ...
    if instr.writes: self.write(result.value, addr_mode(self.MEM[self.pc + instr.num_param]))
    self.pc = result.next_pc if result.next_pc is not None else self.pc + instr.num_param + instr.writes
    if DEBUG:
      postmem = dict(enumerate(self.MEM))
      for k in postmem:
        if not k in premem: print('> MEM', k, postmem[k])
        elif premem[k] != postmem[k]: print('> MEM', k, postmem[k])
